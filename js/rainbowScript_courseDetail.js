...
window.onload = function () {
    ...
    function courseCounter() {
        function createAnimateTag(){
            let tagArea = document.querySelector(".courseImgWrap .courseImg"); 
            let createTag = document.createElement('div');
            createTag.setAttribute('class', 'animateTxt bgc_point01 fc_fff');
            createTag.innerHTML = "마감임박!";

            tagArea.insertBefore(createTag, tagArea.firstChild);
        }

        const today = new Date().getTime();
        const distance = dDay - today;
        const day = Math.floor(distance / (1000 * 60 * 60 * 24));
        localStorage.setItem('dDay[i]', day);

        if (day >= 1) {
            count.innerHTML = "모집 마감 ⏰ D-" + day;
            if (day <= 14){
                // 모집 마감일 2주 전 "마감임박!" 표시하기
                createAnimateTag();
            }
        }else if (day >= 0 && day <= 1) {
